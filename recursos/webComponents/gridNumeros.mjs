export class gridNumeros extends HTMLElement {
	...
	poblarConConjuntos(potencia, presencia) {
		...
		const { x: anchoConjunto, y: altoConjunto } = this.getDimensionesConjunto(potencia);
		...
		const valorEstosConjuntos = Math.pow(10, potencia);

		let guarda = 0;
		for (let i = identificadoresPrimerConjunto.x; i <= identificadoresUltimoConjunto.x; i++) {
			for (let j = identificadoresPrimerConjunto.y; j <= identificadoresUltimoConjunto.y; j++) {
				const numeroEsteConjunto = this.identificadorConjuntoToNumero(potencia, { x: i, y: j });
				if (valorEstosConjuntos * numeroEsteConjunto < this.numero) {
					this.dibujarConjunto(potencia, presencia, { x: i, y: j });
					guarda++;
				}
			}
		}
	...
	dibujarConjunto(potencia, presencia, posicion) {
		...
		padding.x = paddingBase.x + recorridoAsMain * 0.1;
		padding.y = paddingBase.y + recorridoAsMain * 0.1;
		...
		if (potencia === 0) {
			this.lapiz.arc(xFinal + anchoFinal / 2, yFinal + altoFinal / 2, anchoFinal / 2, 0, Math.PI * 2);
		}
		else {
		...
		if (potencia === 0) {
			this.lapiz.fillStyle = `rgba(0, 0, 0, ${opacidad * 0.2})`;
			this.lapiz.fill();
		}
		this.lapiz.stroke();
	}
	dibujarConjuntoByNumero(potencia, numero, presencia) {
		...
		this.dibujarConjunto(potencia, presencia, posicion);
	}
	...
	setEsquinaVisible(posicion) {
		this.esquinaVisible.x = posicion.x;
		this.esquinaVisible.y = posicion.y;
	}
	cambiarZoom(e, centroZoom) {
		//Calcular posición actual del mouse.

		let distanciaCentroEsquina = {
			x: (centroZoom.x - this.esquinaVisible.x),
			y: (centroZoom.y - this.esquinaVisible.y),
		}
		let distanciaCentroEsquinaPx = {
			x: (centroZoom.x - this.esquinaVisible.x) / Math.pow(10, this.zoom / 2),
			y: (centroZoom.y - this.esquinaVisible.y) / Math.pow(10, this.zoom / 2),
		}
		...
		//La distancia del centroZoom a la esquina debe cambiar igual que cambió el zoom.
		let nuevoDistanciaCentroEsquina = {
			x: distanciaCentroEsquina.x / Math.pow(10, cambio / 2),
			y: distanciaCentroEsquina.y / Math.pow(10, cambio / 2),
		}
		let nuevaEsquina = {
			x: centroZoom.x - nuevoDistanciaCentroEsquina.x,
			y: centroZoom.y - nuevoDistanciaCentroEsquina.y,
		}

		this.setEsquinaVisible(nuevaEsquina);
...
#labelZoom{
	...
	color: gray;
		...
		this.redibujar();
		elCanvas.addEventListener("wheel", (e) => { this.handleWheel(e) }, { passive: false });
		this.conectado = true;

		elCanvas.addEventListener("pointermove", (e) => {
			let boundingCanvas = elCanvas.getBoundingClientRect();
			this.mousePos.x = e.clientX - boundingCanvas.left;
			this.mousePos.y = e.clientY - boundingCanvas.top;
		})

		elCanvas.addEventListener("mousemove", (e) => {
			if (e.buttons === 1) {
				this.panVista({ x: e.movementX, y: e.movementY });
			}
		});
	...
	handleClick(e) {
		let potenciaClickable = -Math.ceil(this.zoom);

		let dimensionesConjunto = this.getDimensionesConjunto(potenciaClickable);
		let dimensionesConjuntoZoomed = {
			x: dimensionesConjunto.x * Math.pow(10, this.zoom / 2),
			y: dimensionesConjunto.y * Math.pow(10, this.zoom / 2),
		}
		...
		const lugarClickado = {
			x: this.esquinaVisible.x + posClick.x / Math.pow(10, this.zoom / 2),
			y: this.esquinaVisible.y + posClick.y / Math.pow(10, this.zoom / 2),
		}

		const identificadoresConjuntoClickado = {
			x: Math.floor(lugarClickado.x / dimensionesConjunto.x),
			y: Math.floor(lugarClickado.y / dimensionesConjunto.y),
		}
		console.log(`Clickado el conjunto de potencia ${potenciaClickable} en posición ${JSON.stringify(identificadoresConjuntoClickado)}`);
	}
	getDimensionesConjunto(potencia) {
		const anchoConjunto = 6 * Math.pow(10, Math.ceil(potencia / 2) + 1);
		const altoConjunto = 6 * (Math.pow(10, Math.ceil((potencia + 1) / 2)));
		return {
			x: anchoConjunto,
	...
	panVista(panDelta) {
		this.esquinaVisible.x -= (panDelta.x) / Math.pow(10, this.zoom / 2);
		this.esquinaVisible.y -= panDelta.y / Math.pow(10, this.zoom / 2);
		this.redibujar();
	}
	...
	handleWheel(e) {
		...
		let centroZoom = {
			x: this.esquinaVisible.x + (e.clientX - boundingCanvas.left) / Math.pow(10, this.zoom / 2),
			y: this.esquinaVisible.y + (e.clientY - boundingCanvas.top) / Math.pow(10, this.zoom / 2)
		}

		this.cambiarZoom(e, centroZoom);
	}
}
